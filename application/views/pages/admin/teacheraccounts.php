 	...
 	<div class="w3-container">
  	...
  	<div class="w3-bar-block">  
	    ...
	    <div id="accounts_items" class="w3-container w3-show">
	    	...
	    	<a href="<?php echo base_url();?>admin/teacheraccounts" onclick="w3_close()" class="w3-bar-item w3-button w3-white w3-text-blue w3-hover-text-blue w3-hover-white w3-padding w3-small w3-margin-left" style="border-radius: 30px 0px 0px 30px;">
	    ...
	    <div id="others_items" class="w3-container w3-show">
	    	...
	    	<a href="<?php echo base_url();?>admin/events" onclick="w3_close()" class="w3-bar-item w3-button w3-margin-left w3-hover-text-blue w3-hover-white w3-small" style="border-radius: 30px 0px 0px 30px;"><span class="fa fa-calendar w3-margin-left">	
	    		</span>&nbsp;&nbsp;Events
	    	</a>
...
<!-- | CONTENT | -->
	...
	<header class="w3-container w3-top w3-hide-large w3-blue w3-padding">
		<span>
	    	...
	    	<h3>
	    		<a href="javascript:void(0)" class="fa fa-bars w3-button w3-blue w3-xlarge w3-left w3-hover-light-blue w3-round" onclick="w3_open()"></a>TEACHER'S ACCOUNTS
	    	</h3>
	  	</span>
	</header>
...
<div class="w3-main w3-animate-right w3-margin-bottom" style="margin-left: 245px;">
  	...
  	<div class="w3-container w3-white w3-hide-small w3-hide-medium" style="box-shadow: 2px 0px 5px 1px gray;">
	    <span>
	    	<h3 class="w3-left w3-padding"><b>TEACHER'S ACCOUNTS</b></h3></span>
	    	<div class="w3-dropdown-click w3-right w3-margin-top">
  	...
  	<div class="w3-container">
	    ...
	    <div class="w3-container w3-card w3-padding-large w3-round">
	    	<form action="<?php echo base_url();?>admin/searchTeacher/teacheraccounts" method="post">	
  		...
  		<?php 
  			if(!empty($this->session->flashdata('Teachers'))){
  				...
  				foreach ($res as $resInfo){
  					$t_id = $resInfo['id'];
  					$resTeach = $this->db->query("SELECT * FROM accounts WHERE id = '$t_id'");
	    			foreach($resTeach->result_array() as $result)
  					...
  					echo '<tr>';
	    				...
	    				$showbutton = '<a href="javascript:void(0)" onclick="document.getElementById(\'ViewAccount'.$q_t.'\').style.display=\'block\'" class="w3-small w3-text-blue">View</a> | <a href="'.base_url().'admin/teachAccDeact/'.$result['id'].'" onclick="javascript: return confirm(\'Deactivate account? \')" class="w3-small w3-text-blue">Deactivate</a>';
		    			}else{
		    				$showbutton = '<a href="'.base_url().'admin/teachAccActivate/'.$result['id'].'" onclick="javascript: return confirm(\'Activate account? \')" class="w3-small w3-text-blue">Activate</a>';
		    			}
		    		...
		    		echo '<div id="ViewAccount'.$q_t.'" class="w3-modal">
							<div class="w3-modal-content w3-animate-top" style="max-width:600px">
								<header class="w3-container w3-blue"> 
			        				<span onclick="document.getElementById(\'ViewAccount'.$q_t.'\').style.display=\'none\'" class="w3-button w3-display-topright w3-blue w3-hover-light-blue">&times;</span>
			        					<h3>ACCOUNT LOGIN CREDENTIALS</h3>
		      					</header>
		      					<div class="w3-container w3-margin-left w3-padding">
		      						<center><h4><b>'.$resInfo['fname'].' '.$resInfo['mname'].' '.$resInfo['lname'].'</b> account login credentials.</h4></center>
		      						<div class="w3-left w3-margin-right" style="width:45%;">
					        			<label>Employee number: </label>
					        			<input type="text" class="w3-input w3-border w3-round" value="'.$result['id'].'" disabled>
					        		</div>
					        		<div class="w3-left w3-margin-right w3-margin-bottom" style="width:45%;">
					        			<label>Password: </label>
					        			<input type="text" class="w3-input w3-border w3-round" value="'.$result['password'].'" disabled>
				        			</div>
		      					</div>
	    ...
	    <div class="w3-container w3-card w3-round w3-margin-top">
	    	...
	    	<?php
	    		...
	    		echo "<h6><b>JUNIOR HIGH SCHOOL TEACHERS</b></h6>";
	    		echo '<table class="w3-table-all w3-small" id="listOfStud">';
	    		echo '<thead><tr class="w3-light-grey"><th>Employee number</th><th>First Name</th><th>Middle Name</th><th>Last Name</th><th>Major</th><th>Department</th><th>Acc. status</th><th>Action</th></tr></thead>';
	    		
	    		foreach ($recordJHSTeacher->result_array() as $info) {
	    			$jhsid = $info['id'];
	    			...
	    			if($jhsinfo['account_status'] == "Active"){
	    				$showbutton = '<a href="javascript:void(0)" onclick="document.getElementById(\'ViewAccount'.$i.'\').style.display=\'block\'" class="w3-small w3-text-blue">View</a> | <a href="'.base_url().'admin/teachAccDeact/'.$jhsinfo['id'].'" onclick="javascript: return confirm(\'Deactivate account? \')" class="w3-small w3-text-blue">Deactivate</a>';
	    			}else{
	    				$showbutton = '<a href="'.base_url().'admin/teachAccActivate/'.$jhsinfo['id'].'" onclick="javascript: return confirm(\'Activate account? \')" class="w3-small w3-text-blue">Activate</a>';
	    			}
	    	...
	    	<?php 
	    		...
	    		echo "<h6><b>SENIOR HIGH SCHOOL TEACHERS</h6></b>";
	    		echo '<table class="w3-table-all w3-small" id="listOfStud1">';
	    		echo '<thead><tr class="w3-light-grey"><th>Employee number</th><th>First Name</th><th>Middle Name</th><th>Last Name</th><th>Major</th><th>Department</th><th>Acc. status</th><th>Action</th></tr></thead>';

	    		$j=$i;

	    		foreach ($recordSHSTeacher->result_array() as $info){
	    			$shsid = $info['id'];
	    			...
	    			if($shsinfo['account_status'] == "Active"){
	    				$showbutton = '<a href="javascript:void(0)" onclick="document.getElementById(\'ViewAccount'.$j.'\').style.display=\'block\'" class="w3-small w3-text-blue">View</a> | <a href="'.base_url().'admin/teachAccDeact/'.$shsinfo['id'].'" onclick="javascript: return confirm(\'Deactivate account? \')" class="w3-small w3-text-blue">Deactivate</a>';
	    			}else{
	    				$showbutton = '<a href="'.base_url().'admin/teachAccActivate/'.$shsinfo['id'].'" onclick="javascript: return confirm(\'Activate account? \')" class="w3-small w3-text-blue">Activate</a>';
	    			}
	    		...
	    		echo '</table></div>';
	    	?>
	    	<p><small><b>Note:</b> As DEFAULT, all teacher accounts are INACTIVE until VERIFIED.</small></p>
	    </div>
...
<script type="text/javascript">
    ...
    function LoadAll() {
    	setInterval(function(){
            ...
            notificationitems.open("POST", "https://"+window.location.hostname+"/BanquerohanNationalHighSchool/Admin/notifitems/teacheraccounts", true);
